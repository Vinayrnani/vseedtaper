#!/usr/bin/env node
// verify_v114: B->twister mitre mate preview (apex 167,34,32).
...
(async () => {
  ...
  await page.screenshot({ path: '/home/ubuntu/projects/vseedtaper/screenshots/v114_mitre_mesh.png' });

  // Twister solo (bevel ring detail)
  await page.evaluate(() => { window._setOnlyVisible(['twister']); });
  await page.waitForTimeout(400);
  await page.screenshot({ path: '/home/ubuntu/projects/vseedtaper/screenshots/v114_twister_bevel.png' });

  // Full machine
  await page.evaluate(() => { window._showAllParts(); });
  await page.waitForTimeout(400);
  await page.screenshot({ path: '/home/ubuntu/projects/vseedtaper/screenshots/v114_machine.png' });
  ...
  console.log('verify_v114 PASSED, 0 console errors');
  await pool.releaseBrowser(browser);
