...
(async () => {
  ...
  try {
    ...
    check(assetV === '62', 'ASSET_V=62 got ' + assetV);

    // new parts loaded with meshes?
    const parts = await page.evaluate(() => {
      const out = {};
      ['gear_A', 'gear_Bw', 'gear_Be', 'gear_C', 'bar1', 'barM', 'barE'].forEach(id => {
        let n = 0;
    ...
    // retired v95 parts must NOT be referenced
    const retired = await page.evaluate(() => ({
      defs: (document.documentElement.innerHTML.match(/gear_D|gear_I|'tire'|"tire"|pivotD|pivotI/) || []).length,
      drive: Object.keys(window._drive.pivots)
    }));
    console.log('RETIRED ' + JSON.stringify(retired));
    check(retired.defs === 0, 'no gear_D/I/tire/pivotD/I refs in page');
    check(JSON.stringify(retired.drive.sort()) === JSON.stringify(['A', 'Be', 'Bw', 'C']), 'drive pivots A/Bw/Be/C got ' + retired.drive);

    // pivots near solved positions (root-local viewer coords)
    ...
    check(Math.abs(piv.A[0] - 152) < 1.5 && Math.abs(piv.A[1] - 74.18) < 1.5, 'pivotA ~(152,74.2) got ' + piv.A);
    check(Math.abs(piv.Bw[0] - 141.5) < 1.5 && Math.abs(piv.Bw[1] - 74.18) < 1.5, 'pivotBw ~(141.5,74.2) got ' + piv.Bw);
    check(Math.abs(piv.Be[0] - 164.5) < 1.5 && Math.abs(piv.Be[1] - 74.18) < 1.5, 'pivotBe ~(164.5,74.2) got ' + piv.Be);
    check(Math.abs(piv.C[0] - 159) < 1.5 && Math.abs(piv.C[1] - 60.48) < 1.5, 'pivotC ~(159,60.5) got ' + piv.C);
    ...
    async function rots() {
      return await page.evaluate(() => ({
        ...
        Bw: window._drive.pivots.Bw.rotation.x,
        Be: window._drive.pivots.Be.rotation.x,
    ...
    ratio('A', -2); ratio('Bw', 2.5); ratio('Be', 2.5); ratio('C', -3.125);
    ratio('tw', 7.5); ratio('drum', -0.5);
    ...
    await shot('v96_overview.png', [ch[0] - 300, ch[1] + 170, ch[2] + 300], ch, null, null);
    // 2 gear-train closeup (walls hidden, A/B corner zone, frozen mid-angle)
    const trainOnly = ['gear_A', 'gear_Bw', 'gear_Be', 'gear_C', 'bar1', 'barM', 'barE'];
    await shot('v96_gears.png', [10, 200, 85], [52, 72, 7], trainOnly, Math.PI / 5);
    // 3 pinion-to-wedge closeup (walls hidden, Cpin/twister)
    await shot('v96_pinion.png', [130, 170, 65], [72, 62, 5], trainOnly.concat(['twister']), Math.PI / 5);
    // 4 crank closeup (up position)
    await shot('v96_crank.png', [112, 145, 69], [52, 100, -15], null, null);
