...
// v32 verify: v31 checks + LIVE mesh placement proof (world bounds, not
// constants) for the D1/D2 displacement fixes: hopper tube bottom at
// world 23.9 (child +19.9 re-seat), ribbon top at 24.4 (mesh +0.2),
// 0.5 seal overlap, hopper coaxial on the drum axle.
// Shots: screenshots/v32_*.png (max 25, prune oldest).
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v32_anim_t0.png') });
    await page.evaluate(() => { window._overrideAngle = Math.PI / 2; });
    await page.waitForTimeout(300);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v32_anim_t1.png') });
    ...
    // v32 LIVE placement proof: measure mesh world bounds (root-local).
    // Hopper tube bottom near x 93..107 must sit at 23.9 (child +19.9);
    // ribbon top (flat mesh world max Y) at 24.4 (mesh +0.2); overlap 0.5.
    const place = await page.evaluate(() => {
      ...
      const ribbonTop = box.max.y - rootPos.y + 0; // world->root-local Y
      // hopper cover coaxial: world box centre Y near drum axle 60+?
      // (cover ring centre must equal drum centre 60 within 0.15+box slack:
      // use tube-bottom-derived child offset instead, exact.)
    ...
    check(Math.abs(place.tubeMin - 23.9) < 0.15, `tube bottom world 23.9 (got ${place.tubeMin})`);
    check(Math.abs(place.ribbonTop - 24.4) < 0.06, `ribbon top world 24.4 (got ${place.ribbonTop})`);
    check(Math.abs(place.childOffset - 19.9) < 0.15, `hopper child re-seat +19.9 (got ${place.childOffset})`);
    const liveOverlap = place.ribbonTop - place.tubeMin;
    check(Math.abs(liveOverlap - 0.5) < 0.2, `live seal overlap 0.5 (got ${Math.round(liveOverlap * 100) / 100})`);
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v32_endon_drop.png') });
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(400);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v32_side.png') });
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(400);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v32_top.png') });
    await page.evaluate(() => {
    ...
    console.log('ASSET_V (expect 19):', assetV);
    check(assetV === '19', 'ASSET_V 19 (GLBs rebuilt)');
