...
- **Minimum guesswork, maximum autonomy:** don’t invent facts, but **don’t stall on every gap either**. If a value can be **calculated or derived** (gear sizes from module, clearances from `tol=0.3`, positions from constraints, standard thicknesses/walls), **compute it and use the best engineering approach** — check live HEAD code on disk first — then proceed. State each assumption in one short line in that step’s report so the user can veto it later.
- **Questions are batched, never dribbled:** stop and ask ONLY when something is truly not derivable and guessing wrong would ruin the part (contradicts intent/requirements). When that happens: skip the step, finish all other clear steps, then ask **all pending questions in ONE combined message** (skipped steps + assumptions to confirm) using layman terms/Telugu. Never poke the user mid-queue with one-off questions; the goal is a hands-off experience.
...
- **Purge after each step (context, NOT filesystem):** after a step completes, clear only that step’s **tool outputs/logs from the conversation context** (raw command results, render/screenshot spam, big diffs). **Never compress or drop the user’s own messages or the assistant’s replies/reports** — those stay intact. Goal: lean context for the next step without losing any instruction or decision. **Never** delete repo files or generated artifacts for this — screenshots stay capped via the `screenshots/` gotcha (max 25, prune oldest); nothing is wiped from disk as part of “purge”.
