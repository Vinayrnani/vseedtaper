...
// v14 verify: closed hopper + 2 side joints, half-pipe 16mm/8mm cover 11->6,
// 6 cavities, grooves, bottom-center drop, 6-inch spacing docs.
// Shots: side elevation, mouth closeup, drop closeup, full assembly + anim frame.
// Asserts: 0 console errors; hopper x ~71.5..183; dropSeed x~=100 y 30..30.5;
// crank orbit r=45 about (77.5,60); drum CCW right->top->left->bottom;
// 6 drum seeds present.
(async () => {
  ...
  await page.screenshot({ path: '/tmp/v14_side.png' });
  console.log('Screenshot /tmp/v14_side.png (side elevation)');
  await setView(20, 64, 110, 20, 58, 25);
  await page.screenshot({ path: '/tmp/v14_mouth.png' });
  console.log('Screenshot /tmp/v14_mouth.png (mouth closeup)');
  await showOnly(['hopper', 'cartridge', 'tape']);
  await setView(0, 38, 110, 0, 30, 25);
  await page.screenshot({ path: '/tmp/v14_drop.png' });
  console.log('Screenshot /tmp/v14_drop.png (drop closeup)');
  ...
  await page.screenshot({ path: '/tmp/v14_full.png' });
  console.log('Screenshot /tmp/v14_full.png (full assembly)');
  // animation frame differs (prove motion synced, no random spin: same override delta)
  await page.evaluate(() => { window._overrideAngle = 0.9; });
  await page.waitForTimeout(400);
  await page.screenshot({ path: '/tmp/v14_anim.png' });
  console.log('Screenshot /tmp/v14_anim.png (animation frame)');
  ...
  const layout = await page.evaluate(() => {
    ...
    out.drumSeedCount = drumKids.length ? drumKids[0].children.length : 0;
  ...
  console.log('  drumSeedGroups(6-seed, expect 1):', layout.drumSeedGroups, '| count:', layout.drumSeedCount);
  ...
  const path = await page.evaluate(() => {
    ...
    const inv = root.matrixWorld.clone().invert();
    for (let i = 0; i < 4; i++) {
      window._setRotations(i * Math.PI);
      root.updateWorldMatrix(true, true);
      const seed = window._pivots.drum.children.find(c => c.type === 'Group' && c.children.length === 6).children[0];
      const v = new THREE.Vector3();
  ...
  await browser.close();
  console.log('\nBrowser closed. Shots: /tmp/v14_side.png /tmp/v14_mouth.png /tmp/v14_drop.png /tmp/v14_full.png /tmp/v14_anim.png');
