...
## v75 Step 4: full gear-driven chain (drum→5-stage spur→bevel→ring crown) - 2026-09-19

1. **Why** (user: replace the v72 animated twister with a proper chain-driven gear train from the drum): v72 twister was animated directly (NOT chain-driven). v75 implements the full gear-driven chain: drum 44T → 5-stage compound spur train → 1:1 bevel miter → ring crown 16T, total ratio 12.29× drum (user range 12-18 ✓). The v53 friction/overhead drive is surgically removed. Chassis widened to 64, lengthened to 278, base pocket cut for the ring, feet added, pull_x moved 194→196.
2. **CAD** (`seed_tape_machine_v2.scad` only):
   ...
   - **Crown gear**: 16T M1.5 (pitch r12) on WEST face (x=165), bore Ø17 (r8.5, clears tape tube r5.5 by 3mm). Disc spans x=165-175, y=18-42, z=5-29.
   - **Bobbins**: 2× Class-15 (r10.35, length 11.1mm), at 90° apart, orbit 18, on EAST face (x=181). Bobbin A (front +Y): center (181,48,17), extent y=37.65-58.35, z=6.65-27.35. Bobbin B (top +Z): center (181,30,35), extent z=24.65-45.35, y=19.65-40.35.
   - **Pinion shaft** (axis X, y=49.4, z=9, x=145-164): 12T M1.5 bevel driven at (145,49.4,9) meshes chain shaft E's 12T bevel (axis Z) at same point; 12T M1.5 bevel driving at (164,49.4,9) meshes ring crown 16T (mesh dist from ring center (30,17): sqrt(19.4²+8²)=20.99≈21 ✓). 1mm gap between the two bevels on the same shaft. Bearing blocks at x≈145 and x≈164 from base.
   - **Gear train** (chain from drum, all spur gears M1.5 unless noted): Stage 1 (z=60): drum 44T→11T (cd=55 ✓), shaft A: 11T@60+16T@45. Stage 2 (z=45): 16T→10T, shaft B: 10T@45+16T@30 (cd=19.5). Stage 3 (z=30): 16T→10T, shaft C: 10T@30+16T@15 (cd=19.5). Stage 4 (z=15): 16T→10T, shaft D: 10T@15+16T@12 (cd=19.5). Stage 5 (z=12): 16T→10T, shaft E: 10T@12+12T bevel@9 at (145,49.4) (cd=19.5). Bevel (z=9): shaft E 12T (axis Z)→pinion shaft 12T (axis X) 1:1 miter. Bevel (z=9): pinion shaft 12T (axis X) at (164,49.4,9)→ring crown 16T, ratio 12/16=0.75. TOTAL RATIO: 4×1.6³×1.0×0.75=12.29× drum.
   - **New gears**: 11T M2 (r11), 16T M1.5 (r12), 10T M1.5 (r7.5). All within spur_gear() teeth cap (60).
   - **V53 deletion** (surgical): remove v53 params (~L406-453), v53 asserts (~L496-570), v53 chassis geometry (~L927-986), v53 wall holes (~L1060/L1103). KEEP v72 bobbin/bevel asserts (~L524-532), pull-pin asserts (~L571-573), bore-slip assert (~L562) — update values to pass.
   - Shaft B/C/D positions solved so each stage cd=19.5 exactly, path zigzags from (155,12) to (145,49.4), all shafts/gears stay WEST of x=165, clear of tape lane (tube at y=30,z=17), clear of chassis walls (y=3..61), clear of base pocket (x=165-181).
3. **Viewer** (`web/index.html` only): `ASSET_V` 54→55. New PART_DEFs: twister (ring+crown+bobbins+eyelets), pinion shaft assembly (shaft+2 bevels), shafts A-E with their gears, chains for stages 1-5. Pivots: twister rotates about X at world (173,30,17); pinion shaft rotates about X at (y=49.4,z=9); shafts A-E rotate about Z at their (x,y). Rotation rates: ring 12.29× drum (was 18/7); pinion ring×16/12=ring×1.333; chain each stage per its ratio. GLB URLs bump ?v=.
4. **Collisions**: all gears west of x=165 (clear of ring); tape lane clear (tube y=30,z=17); chassis walls y=3..61 clear; base pocket x=165-181 clear; bobbin A clears front wall (y=61) by 2.65mm and tape tube by 2.15mm; bobbin B clears tube by 2.15mm.
