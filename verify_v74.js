...
// v74 verify (commit 6c1037c feat(v74)) — iteration 2 of gear-chain review.
// v74 fixes: (1) no double-transform on compound/shaft GLB exports,
// (2) compound + shaft visible:true, (3) ASSET_V 55.
// Checks: status, animation, 0 errors, ASSET_V, station accuracy
// (single-transform proof), bore open, chain mesh, ratio 418/27, tape static.
...
(async () => {
  ...
  try {
    ...
    console.log('ASSET_V (expect 55):', assetV);
    check(assetV === '55', 'ASSET_V 55 (v74 GLB rebuild)');
    ...
    // v74 fix 1 — single-transform proof: GLB-local bbox centre must sit
    // near the scene origin (small offset = gear body around its axle).
    // Double-transformed export put geometry ~150mm from scene origin.
    const station = await page.evaluate(() => {
      window._root.updateMatrixWorld(true);
      const out = {};
      ['intermediate_compound', 'drive_shaft'].forEach(id => {
        const arr = window._partMeshes[id] || [];
        const bb = new THREE.Box3();
        arr.forEach(s => s.traverse(o => { if (o.isMesh) bb.expandByObject(o); }));
        const c = bb.getCenter(new THREE.Vector3());
        const s0 = arr[0];
        const sp = new THREE.Vector3(); s0.getWorldPosition(sp);
        const loc = c.clone().sub(sp);
        out[id] = { localOff: +loc.length().toFixed(2),
          world: [+c.x.toFixed(1), +c.y.toFixed(1), +c.z.toFixed(1)] };
      });
      return out;
    });
    console.log('station local-offset: ' + JSON.stringify(station));
    check(station.intermediate_compound.localOff < 25, 'compound on-station localOff=' + station.intermediate_compound.localOff + ' (was ~150 double-transformed)');
    check(station.drive_shaft.localOff < 25, 'shaft on-station localOff=' + station.drive_shaft.localOff + ' (was ~150 double-transformed)');

    // v74 fix 2 — visible in assembled full-machine view (raycast hits them).
    const visib = await page.evaluate(() => {
      window._showAllParts();
      window._root.updateMatrixWorld(true);
      const out = {};
      ['intermediate_compound', 'drive_shaft', 'twister'].forEach(id => {
        const arr = window._partMeshes[id] || [];
        let v = 0, t = 0;
        arr.forEach(s => s.traverse(o => { if (o.isMesh) { t++; if (o.visible) v++; } }));
        out[id] = { visMeshes: v, totalMeshes: t, sceneVisible: arr.length ? arr[0].visible : null };
      });
      return out;
    });
    console.log('visibility: ' + JSON.stringify(visib));
    check(visib.intermediate_compound.visMeshes > 0, 'compound visible:true in assembly');
    check(visib.drive_shaft.visMeshes > 0, 'shaft visible:true in assembly');

    // Bore openness — raycast along world X through twister bbox centre.
    ...
    // Chain connectivity in root-local XY mesh plane (axles run along Z).
    ...
    await shot('v74_full_crankside.png', [CH[0] - 300, CH[1] + 170, CH[2] + 300], CH, null, null);
    await shot('v74_twister_bore.png', [T[0] + 80, T[1] + 6, T[2] + 6], T, ['twister'], 0.6);
    await shot('v74_twister_bevel_west.png', [T[0] - 80, T[1] + 15, T[2] + 25], T, ['twister', 'drive_shaft'], 0.6);
    await shot('v74_gearchain_naked.png', [mid[0] + 40, mid[1] - 60, mid[2] + 150], mid, ['twister', 'intermediate_compound', 'drive_shaft', 'cartridge'], 0.6);
    await shot('v74_gearchain_chassis.png', [mid[0] + 40, mid[1] - 60, mid[2] + 150], mid, ['chassis', 'twister', 'intermediate_compound', 'drive_shaft', 'cartridge'], 0.6);
    await shot('v74_compound.png', [C[0] + 30, C[1] - 40, C[2] + 70], C, ['intermediate_compound'], 0.6);
    await shot('v74_shaft.png', [S[0] + 30, S[1] - 40, S[2] + 70], S, ['drive_shaft'], 0.6);
    await shot('v74_anim_t0.png', [CH[0] - 300, CH[1] + 170, CH[2] + 300], CH, null, 0);
    await shot('v74_anim_t05.png', [CH[0] - 300, CH[1] + 170, CH[2] + 300], CH, null, Math.PI);
