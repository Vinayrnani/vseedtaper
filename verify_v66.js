...
(async () => {
  ...
  try {
    ...
    check(!!st.exitAim && st.exitAim[0] === 172 && st.exitAim[1] === 30 && st.exitAim[2] === 17, 'exit aims dead-on the twister bore machine (172,30,17)');
    ...
    check(!!plowBox && plowBox.sz > 45 && plowBox.sz < 62, `sheet + 2 chassis ears on world z (got z ${plowBox && plowBox.sz})`);
    ...
    await shot('v66_mouth.png', [b.cx - b.sx / 2 - L, b.cy + 8, b.cz], [b.cx - b.sx / 2 + 6, c[1], c[2]], ['plow'], 0.6);
    await shot('v66_exit.png', [b.cx + b.sx / 2 + L, b.cy + 8, b.cz], [b.cx + b.sx / 2 - 4, c[1], c[2]], ['plow'], 0.6);
    await shot('v66_top.png', [b.cx, b.cy + 72, b.cz + 4], c, ['plow'], 0.6);
    await shot('v66_bottom.png', [b.cx, b.cy - 60, b.cz + 4], c, ['plow'], 0.6);
    await shot('v66_near.png', [b.cx, b.cy + 10, b.cz + 62], c, ['plow'], 0.6);
    await shot('v66_far.png', [b.cx, b.cy + 10, b.cz - 62], c, ['plow'], 0.6);
    await shot('v66_iso.png', [b.cx - 40, b.cy + 55, b.cz + 70], c, ['plow'], 0.6);
    // Alignment proof: down the twister bore axis at the folder exit.
    const tw = await page.evaluate(() => window._pivots.twister.position.toArray());
    console.log('twister world pivot:', JSON.stringify(tw));
    await shot('v66_align.png', [tw[0] + 38, tw[1], tw[2]], [b.cx - b.sx / 2 + 4, tw[1], tw[2]], ['plow', 'twister'], 0.6);
    await shot('v66_machine.png', [185, 175, 235], [20, 35, 25], null, null);
    await page.evaluate(() => { window._showAllParts(); window._overrideAngle = null; window._setPanelCollapsed(false); });
    await page.waitForTimeout(600);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v66_anim_t0.png') });
    console.log('Shot: v66_anim_t0.png (animating)');
    await page.waitForTimeout(2500);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v66_anim_t1.png') });
    console.log('Shot: v66_anim_t1.png (animating, +2.5s)');
    ...
    console.log('ASSET_V (expect 50):', assetV);
    check(assetV === '50', 'ASSET_V 50 (v66 twister-aimed feet, plow GLB rebuilt)');
    await page.close();
