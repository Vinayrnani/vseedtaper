...
(async () => {
  ...
  try {
    ...
    check(assetV === '45', 'ASSET_V=45 got ' + assetV);
    ...
    const pivots = await page.evaluate(() => {
      ...
      const out = {};
      ['crankSpinner', 'drum', 'twister', 'takeup'].forEach(k => {
    ...
    check(dDrum < 0, 'drum opposite direction vs crank (DRUM_RATIO=-0.5) got ' + (dDrum/dCrank).toFixed(3));
    check(dTw < 0, 'twister opposite direction vs crank (-3x) got ' + (dTw/dCrank).toFixed(3));
    check(dTu < 0, 'takeup opposite direction (-2x) got ' + (dTu/dCrank).toFixed(3));
    ...
    await shot('rv81_full.png', [ch[0] - 300, ch[1] + 170, ch[2] + 300], ch, null, null);
    await shot('rv81_crank_drum_mesh.png', [130, 80, -120], [130, 50, -30], ['crank', 'cartridge', 'chassis'], null);
    await shot('rv81_anim_t0.png', [ch[0] - 300, ch[1] + 170, ch[2] + 300], ch, null, 0);
    await shot('rv81_anim_t1.png', [ch[0] - 300, ch[1] + 170, ch[2] + 300], ch, null, Math.PI);
