...
// v60: U-to-swirl forming plow — open-U entry dia 21 (180deg, no
// hook, world x126) winding to tight 6-swirl exit dia 9 (330deg +
// 160deg hook, 1.0 daylight gap, world x159) + flat entry tray
// 13.8x16x0.8 west of the large mouth (world 112..125.8).
...
(async () => {
  ...
  try {
    ...
    await shot('v60_plow_front.png', [-5, 20, 29]);
    await shot('v60_plow_back.png', [97, 15, 25]);
    await shot('v60_plow_top.png', [36, 78, 25]);
    await shot('v60_plow_tray.png', [2, 58, 14], [32, 8, 18]);
    ...
    check(hooks.assetV === '44', 'ASSET_V 44 (plow GLB rebuilt)');
    ...
    check(/open[- ]U/.test(hooks.six.entry) && /open[- ]U/.test(hooks.six.stages), 'hooks describe U-to-swirl entry');
    ...
    await page.waitForTimeout(500);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v60_full.png') });
    console.log('Shot: v60_full.png');
    console.log(fails.length ? 'V60_FAIL' : 'V60_DONE');
    if (fails.length) process.exitCode = 1;
  } finally {
    await pool.releaseBrowser(browser);
  }
})().catch(e => { console.error('V60_ERROR', e); process.exit(1); });
