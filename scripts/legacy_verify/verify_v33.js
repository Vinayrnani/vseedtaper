...
// v33 verify: v32 checks adapted to option B (drop tape lower, hover
// pipe). CAD: tape_z 24->13, hover round pipe ID10/OD14 L10 (world
// 23.4..33.4, gap 10 over ribbon top 13.4), tapered groove wide 16->
// 10 throat, shroud roof 34->23 (slot 6.45 kept). Viewer: TAPE_Z 13,
// hopper child +19.4, _dropSeal 23.4/13.4, ASSET_V 20.
// Shots: screenshots/v33_*.png (max 25, prune oldest).
// Asserts: ready, 0 errors, animating, FORM (forming 37..70 west of
// face, transit 70..126 under pipe to plow mouth), HOVER (ID10/OD14/
// L10, gap 10, flange-to-tape >=20, no wheel clash), LIVE placement
// (pipe bottom 23.4, ribbon 13.4), GLB min_z=0 + watertight implied,
// shroud slot 6.45 + roof 23, bend intact, R->L, crank back, ASSET_V 20.
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v33_anim_t0.png') });
    await page.evaluate(() => { window._overrideAngle = Math.PI / 2; });
    await page.waitForTimeout(300);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v33_anim_t1.png') });
    ...
    // full-U at pipe inlet proof: transit verts at x 92..94 reach full height (13+7.5=20.5)
    ...
    check(inlet.top >= 13 + 7.5, `full-U under pipe (top ${inlet.top} >= 20.5)`);
    ...
    // HOVER proof (replaces v32 seal): ID10/OD14/L10, gap 10, flange-to-tape >=20
    ...
    if (seal) {
      check(seal.bore === 10, `ID=10 (got ${seal.bore})`);
      check(seal.outer === 14, `OD=14 (got ${seal.outer})`);
    ...
    // v33 LIVE placement proof: pipe bottom near x 93..107 at 23.4 (child +19.4);
    ...
    // hopper GLB: low-ring outer +-7 (OD14), min_z=0
    ...
    check(Math.abs(hop.loMinX + 7) < 0.15 && Math.abs(hop.loMaxX - 7) < 0.15,
      `pipe outer +-7 OD14 (got ${hop.loMinX}..${hop.loMaxX})`);
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v33_drop_closeup.png') });
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(400);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v33_side.png') });
    await page.evaluate(() => {
    ...
    await page.waitForTimeout(400);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v33_endon.png') });
    await page.evaluate(() => {
    ...
    console.log('ASSET_V (expect 20):', assetV);
    check(assetV === '20', 'ASSET_V 20 (GLBs rebuilt)');
