...
// v43 verify: TRUE-MESHED exterior gear train (decorative-idler audit fix).
...
// (JS mirror of the CAD math) + multi-angle overlap checks + ASSET_V 27.
// Shots: screenshots/v43_*.png (max 25, prune oldest).
...
// v43 mesh table mirror (CAD coords, module 2, pitch r = teeth):
// [nameA, xA, zA, TA, nameB, xB, zB, TB]
const MESHES = [
  ['crank', 40, 60, 20, 'drum', 100, 60, 40],
  ['E0', 40, 60, 12, 'H1', 52.97, 39.81, 12],
  ['H1', 52.97, 39.81, 12, 'R0', 56, 16, 12],
  ['R0', 56, 16, 12, 'C80', 80, 16, 12],
  ['C80', 80, 16, 12, 'C104', 104, 16, 12],
  ['C104', 104, 16, 12, 'C128', 128, 16, 12],
  ['C128', 128, 16, 12, 'C152', 152, 16, 12],
  ['C152', 152, 16, 12, 'H3', 166.73, 34.94, 12],
  ['H3', 166.73, 34.94, 12, 'PC', 190.22, 30, 12],
  ['D2', 100, 60, 20, 'L1a', 130.91, 51.72, 12],
  ['L1b', 130.91, 51.72, 36, 'L2', 166.05, 22.03, 10],
  ['L1b', 130.91, 51.72, 36, 'GT', 178.83, 69.16, 15],
  ['GT', 178.83, 69.16, 15, 'GJ', 205.94, 56.31, 15],
  ['GJ', 205.94, 56.31, 15, 'GS', 226, 34, 15],
];
function meshAudit() {
  const out = [];
...
(async () => {
  ...
  console.log('--- v43 mesh audit (centers vs r1+r2, tol 0.3) ---');
  ...
  // BEFORE (v42 decorative): idler1 (110,60,20T) vs drum (100,60,40T):
  // dist 10 vs 60 (overlap -50); vs crank (40,60,20T): 70 vs 40 (gap +30).
  const before1 = Math.hypot(110 - 100, 60 - 60);
  const before2 = Math.hypot(110 - 40, 60 - 60);
  console.log(`BEFORE idler1-drum: dist ${before1} vs needed 60 (overlap ${before1 - 60})`);
  console.log(`BEFORE idler1-crank: dist ${before2} vs needed 40 (gap +${before2 - 40})`);
  console.log(`BEFORE idler2 (183,17,12T): nearest gear >60 away (decorative)`);
  ...
  try {
    ...
    if (Math.abs(dC) > 1e-4) {
      ...
      check(Math.abs(d.tk / dCrank - 2) < 0.05, `takeup +2x crank (v43 sense-reversed, got ${d.tk / dCrank})`);
    ...
    // v43 animating snapshots: back-wall gear train closeups + full proofs.
    ...
    const shots = [
      { f: 'v43_train_west.png', only: ['chassis'], cam: [0, 60, 220], tgt: [-30, 45, 68], ang: 0.6 },
      { f: 'v43_train_east.png', only: ['chassis'], cam: [110, 55, 220], tgt: [80, 40, 68], ang: 0.6 },
      { f: 'v43_train_full.png', only: ['chassis'], cam: [30, 50, 320], tgt: [30, 45, 66], ang: 2.4 },
      { f: 'v43_bind_closeup.png', only: ['plow', 'twister', 'pull_a', 'pull_b', 'tape', 'seeds'], cam: [60, 30, 60], tgt: [72, 17, 25], ang: 0.6 },
      { f: 'v43_side.png', only: null, cam: [17, 45, 430], tgt: [17, 45, 27], ang: 0 },
      { f: 'v43_endon.png', only: null, cam: [430, 55, 27], tgt: [17, 45, 27], ang: 0 },
      { f: 'v43_top.png', only: null, cam: [17, 430, 27], tgt: [17, 45, 27], ang: 0 }
    ];
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v43_anim_t0.png') });
    console.log('Shot: v43_anim_t0.png (animating)');
    await page.waitForTimeout(1200);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v43_anim_t1.png') });
    console.log('Shot: v43_anim_t1.png (animating, later timestamp)');
    ...
    console.log('ASSET_V (expect 27):', assetV);
    check(assetV === '27', 'ASSET_V 27 (GLBs rebuilt)');
    await page.close();
