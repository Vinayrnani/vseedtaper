...
// v42 verify: TRUE 6-fold second stage (two-stage: first-stage U 37..126,
// 6-turner 126..159 folds the 2 U edges INSIDE via twin horns + inner tongue).
// Stations frozen: turner 126..159 (lip 160.35) -> bind 172 (168..176) ->
// pull 194 (183..205 incl. caps) -> take-up 226/34 (210..242);
// edge gaps 7.65/7/5. Checks: animating + gear-only sync, _sixTurner
// stages/entry/exit hooks, GLB min_z=0, vertex clearances >=4.5,
// tape 270, ASSET_V 26. Shots: screenshots/v42_*.png (max 25, prune oldest):
// side/end-on/top closeups of the turner showing U-in then rolled-out.
...
(async () => {
  ...
  try {
    ...
    // v42 hooks: two-stage 6-turner (U-in, twin horns + tongue fold edges inside)
    ...
    if (hooks.six) {
      ...
      check(hooks.six.dropX === 100 && hooks.six.start - hooks.six.dropX >= 8, `turner AFTER drop (mouth-drop=${hooks.six.start - hooks.six.dropX} >= 8, seed lands flat/open-U first)`);
      check(hooks.six.stages === 'U-in then 6-fold edges inside to roll', `two-stage hook (got ${hooks.six.stages})`);
      check(hooks.six.entry === 'U-accept' && hooks.six.exit === 'near-closed tube', `entry U-accept / exit near-closed tube (got ${hooks.six.entry}/${hooks.six.exit})`);
    }
    ...
    // 3 crank angles. Design X gaps: turner-twister 7.65 (lip 160.35),
    // twister-pull 7, pull-takeup 5. Threshold 4.5 proves the 5.0 gap.
    ...
    // v42 multi-angle animating snapshots: turner two-stage closeups
    // (U-in then rolled-out) + full-machine proofs.
    // NOTE: world coords = root(-100,0,55) + root-local; machine spans
    // world x -114..148 (centre 17), y 0..110 (centre 45), z -5..63.
    // Turner world centre ~ root-local (142,10,-30) -> world (42,10,25).
    const shots = [
      { f: 'v42_turner_side.png', only: ['plow', 'twister', 'tape', 'seeds'], cam: [0, 30, 100], tgt: [42, 14, 45], ang: 0.6 },
      { f: 'v42_turner_endon.png', only: ['plow', 'tape', 'seeds'], cam: [170, 16, 25], tgt: [42, 12, 25], ang: 0.6 },
      { f: 'v42_turner_top.png', only: ['plow', 'twister', 'tape', 'seeds'], cam: [42, 140, 28], tgt: [42, 10, 25], ang: 0.6 },
      { f: 'v42_bind_closeup.png', only: ['plow', 'twister', 'pull_a', 'pull_b', 'tape', 'seeds'], cam: [60, 30, 60], tgt: [72, 17, 25], ang: 0.6 },
      { f: 'v42_pull_wind.png', only: ['pull_a', 'pull_b', 'takeup', 'twister', 'tape', 'seeds'], cam: [40, 70, 110], tgt: [105, 22, 25], ang: 2.4 },
      { f: 'v42_side.png', only: null, cam: [17, 45, 430], tgt: [17, 45, 27], ang: 0 },
      { f: 'v42_endon.png', only: null, cam: [430, 55, 27], tgt: [17, 45, 27], ang: 0 },
      { f: 'v42_top.png', only: null, cam: [17, 430, 27], tgt: [17, 45, 27], ang: 0 }
    ];
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, 'v42_anim_t0.png') });
    console.log('Shot: v42_anim_t0.png (animating)');
    await page.waitForTimeout(1200);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v42_anim_t1.png') });
    console.log('Shot: v42_anim_t1.png (animating, later timestamp)');
    ...
    console.log('ASSET_V (expect 26):', assetV);
    check(assetV === '26', 'ASSET_V 26 (GLBs rebuilt)');
    await page.close();
