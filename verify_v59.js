...
// v59: photo-matched 6-folder — v58 big-to-small shell (entry dia 21
// world x126 -> exit dia 9 world x159, 1.0 daylight gap) + flat entry
// tray 13.8x16x0.8 west of the large mouth (world 112..125.8).
...
(async () => {
  ...
  try {
    ...
    await shot('v59_plow_front.png', [-5, 20, 29]);
    await shot('v59_plow_back.png', [97, 15, 25]);
    await shot('v59_plow_top.png', [36, 78, 25]);
    await shot('v59_plow_tray.png', [2, 58, 14], [32, 8, 18]);
    ...
    check(hooks.assetV === '43', 'ASSET_V 43 (plow GLB rebuilt)');
    ...
    await page.evaluate(() => {
      window._camera.position.set(60, -120, 90);
      window._controls.target.set(60, 10, 20);
      window._controls.update();
    });
    await page.waitForTimeout(500);
    await page.screenshot({ path: path.join(SHOT_DIR, 'v59_full.png') });
    console.log('Shot: v59_full.png');
    console.log(fails.length ? 'V59_FAIL' : 'V59_DONE');
    if (fails.length) process.exitCode = 1;
  } finally {
    await pool.releaseBrowser(browser);
  }
})().catch(e => { console.error('V59_ERROR', e); process.exit(1); });
